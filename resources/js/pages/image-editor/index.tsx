...
import { ArrowLeft, ImagePlus, PenLine, Plus } from 'lucide-react';
import { useState } from 'react';

import { SourcePicker } from '@/components/image-editor/source-picker';
import { SourceUpload } from '@/components/image-editor/source-upload';
...
export default function ImageEditorIndex({ sessions, generatedImages }: Props) {
    const [activeTab, setActiveTab] = useState<SourceTab>('upload');
    const [showNewSession, setShowNewSession] = useState(false);

    function openSession(session: ImageEditSession) {
                ...
                <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6">
                    ...
                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                        ...
                        <button
                            type="button"
                            onClick={() => setShowNewSession(true)}
                            className="flex aspect-square flex-col items-center justify-center gap-3 rounded-2xl border-2 border-dashed border-border/60 transition-colors hover:border-amber-accent/40 hover:bg-amber-accent/5"
                        >
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-4 md:p-6 lg:flex-row">
                    ...
                    <div className="overflow-hidden rounded-2xl border border-border/60 bg-card">
                        ...
                        <div className="flex items-center gap-3 border-b border-border/60 px-6 py-5">
                            <div className="flex size-9 items-center justify-center rounded-xl bg-gradient-to-br from-amber-accent/20 to-amber-accent/5">
                                <ImagePlus className="size-4.5 text-amber-accent" />
                            </div>
                            <div className="flex-1">
                                ...
                                <p className="text-xs text-muted-foreground">
                                    Edit, enhance, and transform your images
                                </p>
                            </div>
                            {sessions.length > 0 && (
                                <button
                        ...
                        <div className="p-6">
                            ...
                            <div className="mt-4">
                                {activeTab === 'upload' ? (
                                    ...
                                    <SourcePicker images={generatedImages} />
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-border/60 p-16 text-center">
                    ...
                    <p className="mt-1.5 max-w-xs text-sm leading-relaxed text-muted-foreground">
                        Upload an image or select one from your generated images
